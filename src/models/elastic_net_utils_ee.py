...
def load_temporal_dataset_ee(path: Path) -> pd.DataFrame:
    """Load one temporal composition dataset and standardize it to t1/t2 names."""
...
def build_elastic_net_pipeline_ee(
    ...
    return Pipeline(
        steps=[
            ...
            ("scale_features_ee", StandardScaler()),
            ("model_ee", model),
        ]
    )
...
def row_sum_summary_ee(frame: pd.DataFrame) -> dict[str, float]:
    """Summarize how close row-wise proportions are to summing to 1."""
...
def compute_regression_metrics_ee(
    ...
    """Compute target-wise and overall regression metrics."""
